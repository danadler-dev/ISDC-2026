...
\begin{abstract}
Many domains describe complex change as “evolutionary,” yet most system dynamics models remain constrained to fixed state spaces, static transition rules, and equilibrium-seeking behavior. As a result, they capture adaptation within predefined structures but struggle to represent the emergence of new structures, strategies, or categories over time. This paper argues that evolutionary dynamics should be treated as a core systems-thinking problem rather than a biological metaphor.

We introduce Stability-Driven Assembly (SDA) as a minimal dynamic framework in which stochastic interactions combined with differential persistence generate endogenous selection without requiring genes, replication, or predefined fitness functions. In SDA, patterns that persist longer become over-represented in the population and therefore more likely to participate in future interactions, creating feedback between population composition and system dynamics. This feedback implements fitness-proportional sampling as an emergent property of the system, yielding a natural genetic-algorithm-like process driven solely by stability.

Using SDA as a concrete example, we show how evolutionary dynamics arise from non-equilibrium feedback rather than equilibrium optimization, and why simulation-based approaches are essential when the effective state space and transition structure evolve over time. We argue that models constrained toward equilibrium or fixed transition rates cannot capture open-ended evolution, even when simulated numerically. The paper concludes by discussing implications for system dynamics, economics, and policy modeling, and by suggesting that agent-based and AI-augmented systems may provide a practical substrate for evolutionary system models in which strategies, representations, and state spaces co-evolve.
\end{abstract}
